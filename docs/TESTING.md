...
## Current Coverage (857 tests, 49 files + module_packages/journal/tests/)
...
| `test_home_module_conversion.py` | 4 | `m016`'s migration guard, deliberately different from journal's — keyed on `ha_config.json`'s own existence *and* a real url+token (not journal's blanket "instance existed before" signal), plus install→uninstall→reinstall preserving favourites |
