...
## Current Coverage (968 tests, 58 files + module_packages/{journal,automations,household,team,chat,notes,dashboard,assets}/tests/)
...
| `test_dashboards_service.py` | 36 | Dashboard CRUD/access, template sync/detach, sharing, `stacked_layout()`'s bottom-stack math, cross-workspace visibility — stays core, `dashboards_service.py` never moved when dashboard/ converted (2026-08-27). Lost its 5 Collection-block tests (+ the `_collections` import) when assets/ converted the same day — moved to `module_packages/assets/tests/test_assets_dashboard_block.py` below, since the resolver itself folded into `module_packages/assets/backend/dashboard_block.py` |
