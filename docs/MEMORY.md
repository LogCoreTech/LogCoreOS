...
**How to apply:** When adding a module, update both `constants.js` AND the backend router's `require_module()` call. Current IDs: `dashboard`, `tasks`, `calendar`, `household`, `notes`, `journal`, `chat`, `automations`, `automations_business`.
