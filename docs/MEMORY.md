...
- **2026-07-06** — **Product milestones v0.2→v1.0 adopted** (from the mobile roadmap/vision session), mapped to business phases in BLUEPRINT.md; the revenue rule still decides sequencing. Backlogged by that rule: quick capture, offline-first sync, journal insight loops. The v1.0 trust stack (app 2FA, automated backups+restore, audit logging, <10-min onboarding) gates the Show HN post. Roadmap-doc corrections: proactive digest, per-workspace AI context, basic recurring tasks, workspaces/Team/HA/n8n were already shipped.
