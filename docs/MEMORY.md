...
`ALL_MODULES` in `app/frontend/src/lib/constants.js` defines the module registry. Each module `id` must exactly match the string used in `require_module(module_id)` on the backend.
**Why:** Mismatch causes silent access failures or broken feature gating.
**How to apply:** When adding a module, update both `constants.js` AND the backend router's `require_module()` call. IDs are: `dashboard`, `tasks`, `calendar`, `household`, `notes`, `journal`, `chat`.
