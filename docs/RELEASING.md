...
7. **Tag** `vX.Y.Z` (lowercase `v`) on that commit — `git tag -a vX.Y.Z -m "vX.Y.Z"`.
8. **Generate a GitHub release title and body for the owner to copy** when creating the release on GitHub:
   ...
   - Present this directly in the conversation for the owner to copy — don't create the GitHub release yourself (that's a `gh release create` action against the real public repo, outside what a commit/tag needs and not something to do without being asked).
9. **Don't push** the commit or tag unless explicitly asked — commits are session-scoped (see memory: commits-are-session-scoped), and a tag push is the same category of action.
