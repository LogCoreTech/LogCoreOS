...
- **Invoicing (Phase D, `finance_invoice_service.py` + `routers/finance_invoicing.py`)**: clients (`clients.json` per book; **`contact_id` reserved for the future CRM module, never populated here**; delete 409s while invoices exist — archive instead); invoices (`invoices.json`; auto-numbered via `finance_service.next_invoice_number()` from book `invoice_prefix`+`invoice_seq`; `status` stores ONLY the user-set lifecycle draft|sent|paid|void — totals/paid/balance/**overdue** are computed in `annotate_invoice()`, never stored; full payment auto-flips to `paid`, removing a payment reopens to `sent`); partial `payments[]`, each optionally creating a **linked income transaction** (`tx_id` ↔ tx `invoice_id`/`client_id`); `ar_summary()` = per-client invoiced/paid/outstanding/overdue rollup, worst first ("who's behind"); voided invoices excluded. **Receipts** on transactions mirror the assets attachment pattern (`receipts/{tx_id}/{uuid}.{ext}`, uuid disk names, 10 MB, ≤10/tx, MIME allowlist, deleted with the transaction). **Reports**: `pnl()` (year/quarter/month) + `tax_summary()`/`tax_summary_csv()` (deductible flag + per-book `tax_categories` buckets) — all in `finance_reports.py`; **`month_end()` for month ranges — never fabricate day 31** (Feb/Apr bug class). Invoice PDF = client-side print CSS (`InvoicePrint`), zero server deps. AI write tools `create_invoice`/`mark_invoice_paid` (approval-gated).
