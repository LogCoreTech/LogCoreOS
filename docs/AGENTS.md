...
- **Blocks**: a dashboard is an ordered list of typed blocks (`{id, type, config, layout: {lg, sm}}`), laid out on a freeform `react-grid-layout` grid (drag/resize disabled below the `sm` breakpoint — see the mobile-PWA risk note in MEMORY.md). The block-type catalog lives in `services/dashboard_blocks/registry.py`'s `REGISTRY` dict (backend, gates access) mirrored by `components/dashboard/blockRegistry.js` (frontend, supplies render components) — 27 types across Tasks/Goals, Smart Home, Household/Team pools, Calendar, Finance, Contacts, Assets, Notes, Journal, Automations, AI Chat/Usage, and Freeform (Text/Link/Divider).
...
- **Not yet built** (deliberately deferred, see `docs/TASKS.md`): dashboard templates, ANY "Referenced by" UI (the backend endpoint/index works, no frontend surface calls it yet), Module Engagement + External Data block types, and Net Worth/Spending/Completion trend blocks (need new aggregation endpoints — `finance_reports.py` only computes single-period snapshots today).
