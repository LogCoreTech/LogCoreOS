...
- **Comments**: append-only attributed log per asset (`comments`, cap 100, text ≤2000). Posting = edit-level or contribute-with-comments-cap; read access is view-only. Every comment notifies edit-level users (owner + accepted edit shares; pool: admins + pool_edit grantees) except the author, via `suggestions_service.notify_user()` — in-app notification with `{type:"open_asset", asset_id}` action (NotifBell renders **View →**) plus ntfy/web push deep-linking to `/assets?asset=<id>` (Assets.jsx consumes the query param and opens the read-first view).
