...
>
> **Remediation pass 3 — 2026-07-20 (infra, owner chose full socket-proxy). NOT smoke-tested — no Docker daemon in the build env; requires a real-deploy verification (see below).**
> - ✅ **B/CRITICAL container-escape** — added a `docker-socket-proxy` service (mounts the socket read-only, internal TCP only, no published port); the app now talks Docker via `DOCKER_HOST=tcp://socket-proxy:2375` and the raw socket bind + `group_add` were removed from the app container. Proxy allows only `CONTAINERS`/`IMAGES`/`POST` (+ start/stop/restart); `EXEC`, `VOLUMES`, `NETWORKS`, `SWARM`, `SYSTEM`, `INFO`, etc. are denied. **Residual:** the Admin→Hosting tunnel flow calls `containers.run()` (creates a container), so `POST`/create must stay enabled — a determined app-RCE could still craft a host-mounting container. Fully closing this needs a fixed-command **broker** (recommended follow-up), or refactoring the tunnel to a pre-declared compose container so create can be denied.
> - ✅ **B/HIGH weak n8n defaults** — `launch.sh` now generates strong random `N8N_API_KEY` + `N8N_ENCRYPTION_KEY` for **new** installs; n8n/ntfy ports bound to `127.0.0.1`. The weak compose literals are kept ONLY as backward-compat fallbacks with a warning — **`N8N_ENCRYPTION_KEY` must never be rotated on an instance with existing n8n data** (it decrypts stored credentials).
> - ✅ **B/MEDIUM image pinning** — `cloudflared:2026.7.2`, `ntfy:v2.26.0`, `n8n:2.31.3`, `docker-socket-proxy:0.3.0`.
>
...
>
> **Remediation pass 5 — 2026-07-20 (LOW items). Tested — scripts parse (`bash -n`), signing gate unit-exercised, frontend builds on the new Vite.**
> - ✅ **LOW: unsigned `update.sh` pull** — the updater now `fetch`es, verifies, then fast-forwards (no blind `git pull`/merge). With `UPDATE_REQUIRE_SIGNATURE=true` it refuses any commit lacking a valid trusted GPG signature (commit or annotated tag) before building; default off for unsigned deployments. Documented in `docker/.env.example`.
> - ✅ **LOW: `vite` staleness** — bumped `^5.3.1` → `^5.4.21` (closes the Vite dev-server path-traversal advisories); frontend build verified, assets still emit to `dist/static/`. Residual **esbuild** dev-server advisory needs a breaking Vite 8 major — deferred (dev-only, production serves the built bundle). ESLint 9 flat-config migration also deferred.
> - ✅ **LOW: `curl|sh` installer** — documented in `launch.sh` (`--install-deps` now warns it runs vendor bootstrap scripts as root, trust-on-first-use, and points to the manual-install path). Left as-is by design — it's the standard vendor pattern and opt-in.
>
> **Still open (deferred):** **2FA** — the other residual, a real feature on the v1.0 backlog that needs its own design pass.
