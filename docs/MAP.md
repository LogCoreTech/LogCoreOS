...
│   │   ├── module_registry.py    → Mod Store universal module system: `ModuleManifest` dataclass, `discover_manifests()`/`active_manifests()` (importlib-based, scans `module_packages/*/manifest.py`), `register_routers()` (asymmetric failure handling — optional modules log-and-skip, `uninstallable` ones crash boot loudly), `brain_paths_for_disabled()` (shared with `agent_service.py`, since services never import from routers). SECURITY: this executes arbitrary Python — safe only because module_packages/ is first-party reviewed code, never community-submitted (see its own docstring)
│   │   ├── module_packages/      → converted modules live here, one dir per id — journal/ is the first, fully converted 2026-08-24 (backend + frontend): manifest.py (ModuleManifest incl. m015 upgrade migration + on_install/on_new_user backfill hooks + help_section), backend/{router,service,dashboard_block,agent_tools}.py, tests/test_journal_service.py. Frontend counterpart lives at app/frontend/src/module_packages/<id>/ (manifest.js + frontend/{Page,BlockComponent,api}.jsx|js) — discovered by lib/moduleRegistry.js, see the frontend section below
