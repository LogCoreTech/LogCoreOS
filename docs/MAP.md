...
│   │   ├── module_packages/      → converted modules live here, one dir per id — journal/ is the first (backend half done 2026-08-24, frontend pending): manifest.py (ModuleManifest incl. m015 upgrade migration + on_install/on_new_user backfill hooks + help_section), backend/{router,service,dashboard_block,agent_tools}.py, tests/test_journal_service.py
