...
- **Admin: reset a user's password.** Generates a random temporary password (shown once, copy it to relay to the user) and requires them to set their own the next time they use it. Also adds the self-service **"Reset Password" popup in Settings → Account** that both this reset flow and any user wanting to rotate a known password now use — no path existed for either before.
- **Self-service email change** — a new "Update Email" popup in Settings → Account lets you change your own login email (confirmed with your current password), with no admin involvement needed.
- **Settings → Account redesign**: email and password are now shown as fixed account facts (email displayed, password masked) with "Update Email"/"Reset Password" buttons that open a focused popup, instead of always-open editable fields on the page itself.
- **Trash gained a back button** — it was previously a dead end with no way back except the browser's own back button. Returns you to wherever you opened it from (Settings, or a module's own "View trash" link), not a fixed destination.
...
- **New: Homes module.** Manage your rented or owned homes — each one gets its own tag the moment you create it, and anything you tag with it elsewhere (a task, note, calendar event, transaction, asset, or contact) shows up automatically on that home's own page, organized into Overview/Tasks/Notes/Money/Events tabs. Rent and own each have their own relevant fields (lease dates and rent, or purchase details and mortgage payment), and a home can optionally be shared with your whole household or team instead of kept personal.
- **A personal home can now be shared with your household/team after the fact** — a "Share with Household/Team" button on the home's own page, not just a checkbox at creation time. One-way, same as converting a personal contact or asset to shared.
- **Homes' Events tab (and its Overview preview) now hides tagged calendar events that have already happened**, instead of accumulating every past event forever.
- **Homes: quick maintenance/warranty reminders.** A "+ Reminder" picker on a home's Tasks tab offers common presets (HVAC filter monthly, gutter cleaning and smoke detector batteries every 6 months, water heater flush yearly) that pre-fill a real task for you to review before saving — or pick Custom to start blank, still tagged to the home automatically.
- **A "Home Health" dashboard block** — add it for any one home to see its lease/mortgage countdown, open and overdue task counts, upcoming tagged events, and this month's tagged spending, all in one glance.
- **Homes gained Assets and Contacts tabs.** Tag an asset (including one with an attached PDF — a lease, deed, or insurance policy) or a contact (a preferred plumber, electrician, etc.) with a home's tag and it now shows up right on that home's own page, alongside its tasks, notes, and events.
- **A home is now findable by name or address from the app-wide search bar itself**, not just the reverse direction (tagging other things with it).

### Fixed

- **Clicking "Add transaction" (or editing one) in Finance crashed outright.** A pre-existing bug, unrelated to anything else in this release — a helper used for the "warn before discarding unsaved changes" feature referenced two fields before they were set up, which JavaScript treats as an error rather than quietly ignoring. Reordered so it works every time now.
- **6 more numeric fields across the app** (a goal's manual progress entry, a recurring task/reminder's day-of-month and repeat-every count, a metric's target value, notification digest hour/day settings, a custom asset field, and a contact's height/weight) **could silently refuse to save if you typed something the field didn't expect** — the exact same class of bug as the Homes comma issue: the browser blocks the whole form the instant the text isn't a plain number, with no error shown at all. All switched to the same safer text-entry pattern.
...
- **App-level two-factor authentication (2FA).** Turn it on for your own account in Settings → Account — scan a QR code with any authenticator app (Google Authenticator, Authy, etc.), confirm a 6-digit code, and save the 10 one-time recovery codes shown. From then on, logging in asks for that code (or a recovery code) after your password. Admins can also require it instance-wide — off, admins only, or everyone — from Admin → General, and can reset a user's 2FA from their User Detail page if they lose access to their authenticator app. An admin must enroll themselves before they can require it for anyone else, so no one can lock themselves out by flipping the switch. When 2FA is required for you, you can't turn it back off yourself — only an admin reset, or the requirement itself being lifted, can remove it. (2FA originally shipped on its own "Security" settings page; folded into Account the same day per owner feedback that it didn't need a whole separate page.)
...
- **Fixed a real bug**: the first-run Getting Started checklist could link a restricted account to Tasks or Chat even when those were disabled for their role — it now only shows steps for modules you actually have access to. A dashboard with genuinely nothing to show (limited module access) now says so instead of just looking blank.
- **Empty states on Tasks, Notes, and Contacts** now have a clear call-to-action instead of just a plain message.
- **A "Duplicate" button on Tasks, Calendar events, and Finance transactions** — makes a copy with everything the same except completion status/streaks, in the same personal or shared scope as the original.
- **"Save & add another" on Finance's Add Transaction form** — keeps the account/category/date selected so entering several transactions in a row doesn't mean re-picking them every time.
...
- **New instances now start with Journal, Calendar, Notes, and Goals already installed**, alongside the always-on Tasks/Chat/Dashboards — previously a fresh instance started with only those 3, leaving every other module to be discovered cold in Admin → Mod Store.
- Every confirmation dialog across the app (delete, leave, disconnect, restart, etc.) now uses one consistent, accessible styled component instead of the browser's plain `confirm()` popup. Every modal in the app can now also be closed by pressing Escape.
...
- **ntfy has been removed entirely.** Web Push is now the app's only push notification channel — it already carried the real notification load in parallel with ntfy since 0.7.0, so this drops a redundant, harder-to-use second channel rather than losing delivery. Settings → Notifications no longer shows an ntfy card; the standalone `ntfy` Docker service, its published port, and its `NTFY_*` env vars are gone from `docker-compose.yml`/`.env.example`. Updating past this release automatically stops and removes the old `logcore-ntfy` container (`docker compose up --remove-orphans`, run by both `launch.sh` and the auto-updater) — its data volume is left alone; remove it yourself with `docker volume rm docker_ntfy_data` if you want the disk space back. A one-time migration strips the now-unused notification-channel fields from `auth.json` on upgrade. **If you rely on a device that can't use Web Push** (no PWA installed, or an old/unsupported browser), you'll only get in-app/chat notifications on that device going forward — there's no fallback channel anymore.
...
- **The updater can now recover on its own from a "divergent local history" failure**, instead of needing you to SSH in and fix it by hand. If your instance's history no longer shares commits with the latest release (rare — happens if the project's own published history is ever rewritten upstream), Admin → Updates now shows exactly what happened with a "Fix Automatically" button: it explains the situation, links to GitHub's own compare view so you can see exactly what's changing first, and — only once you confirm — resets your instance's copy of the app's code to match the release. Your Brain data is never touched.
