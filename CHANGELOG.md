...
- **Custom Dashboards** — the fixed Dashboard page is replaced with an unlimited, build-it-yourself dashboard system. Create as many dashboards as you want, each a freeform, drag-and-resize grid (desktop) of blocks pulling live data from almost every module: tasks, goals, streaks, Smart Home, Household/Team, Calendar, Finance, Contacts, Assets, Notes, Journal, Automations, and your own AI usage/recent activity — plus plain text, links, and headings for organizing the layout. Share a dashboard with another person or your household/team pool at read, contribute, or edit level; an optional owner-only toggle lets you share the underlying data too, so someone you've shared with sees exactly what you'd see — never more. Your existing dashboard becomes your first "Home" dashboard automatically; your last remaining dashboard in a workspace can't be deleted, so there's always a landing page. Dashboard templates, "referenced by" jump-links back from other modules, admin-only usage-analytics blocks, and external data sources (weather, RSS, etc.) are still on the way.
