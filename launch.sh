...
launch_containers() {
  ...
  ensure_n8n_env
  # --remove-orphans stops/removes any running container whose service no
  # longer exists in docker-compose.yml (e.g. logcore-ntfy on an instance
  # upgrading past the 2026-09-06 ntfy removal) instead of leaving it running
  # unmanaged forever. Its data volume (ntfy_data) is left alone — Compose
  # never removes volumes on its own, and deleting one is a real, destructive
  # call this script shouldn't make unattended; see docs/MEMORY.md's
  # 2026-09-06 entry for the one-line manual cleanup if you want it gone.
  docker compose \
