"""Contacts (CRM) module — core: contact CRUD, self-contact/profile,
pipeline, custom fields, CSV import/export, per-contact photo, and the
cross-module finance summary. Interactions/deals live in router_deals.py,
access/sharing (including the member/role pickers) in router_sharing.py, and
the write-focused n8n automation API in router_automation.py —
module_packages/contacts/manifest.py's `_get_router()` composes all four into
one router, mirroring module_packages/finance/backend/router.py's own split
exactly.

Access is resolved server-side via contacts_service.find_contact(); the
frontend never decides it. contribute = log interactions + create/advance
deals only (never edit core fields / delete / reshare) — enforced here and in
router_deals.py.
"""
