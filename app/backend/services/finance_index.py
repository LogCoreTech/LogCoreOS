"""Derived share-routing cache for finance books (mirrors assets_index.py).

Maps owner → workspace → share-target tokens so list_visible_books/find_book
only scan stores that could contain something shared with the viewer.
NEVER a source of truth — rebuildable from Brain files at any time; warmed at
startup and updated incrementally on access writes.
"""
...
def _index_path():
    return brain_path() / "_system" / "finance_share_index.json"


def _collect_targets(books: list[dict]) -> list[str]:
    tokens: set[str] = set()
    for book in books:
        for entry in book.get("shared_with") or []:
            if entry.get("target"):
                tokens.add(entry["target"])
        for account in book.get("accounts") or []:
            for entry in account.get("shared_with") or []:
                if entry.get("target"):
                    tokens.add(entry["target"])
...
def _scan_owner(owner: str) -> dict:
    ...
    for workspace in ("personal", "business"):
        books = finance_service.list_books(owner, workspace)
        tokens = _collect_targets(books)
        if tokens:
