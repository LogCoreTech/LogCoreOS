...
def _brain_skip(user: dict) -> set[str]:
    ...
    Dashboards/Assets/Contacts folders (structurally different shape,
    JSON+binary files, not markdown)."""
    from module_registry import brain_paths_for_disabled

    disabled = set(user.get("disabled_modules", []))
    return {"Tasks", "Dashboards", "Assets", "Contacts"} | brain_paths_for_disabled(disabled)
