"""Contacts (CRM) core: contacts, custom fields, sharing/access resolution,
self-contact/profile, and cross-workspace visibility. Deals + interactions
CRUD, the pipeline/stages logic, and deal<->asset linking live in
contacts_deals_service.py (split out 2026-09-16) and are re-exported below
...
# (mostly `from services import contacts_service` + `contacts_service.X(...)`
# attribute access, across the router/agent tools/finance invoice service/
# tests) keeps working unchanged — a full call-site migration would have
# touched a couple dozen call sites across files that also use plain-contact
# functions from this module, for no behavior change. contacts_deals_service
# itself reaches back into THIS module (for get_contact/resolve_access/
# _candidate_stores/_now, since a deal has no access of its own) via
# function-local imports, so this is not a circular import.
from services.contacts_deals_service import (  # noqa: F401
    DEFAULT_STAGES,
    INTERACTION_TYPES,
    _list_deals,
    _list_interactions,
...
# _list_interactions / _save_interactions / _list_deals / _save_deals now
# live in contacts_deals_service.py (imported above) — delete_contact(),
# transfer_ownership(), and the follow-up sweep below still call them
# directly to cascade/move/scan a contact's interactions + deals.
...
# get_pipeline / set_pipeline now live in contacts_deals_service.py (imported
# above); _validate_deal() there calls get_pipeline() directly.
...
# Interactions CRUD (list_interactions/add_interaction/update_interaction/
# delete_interaction) and Deals CRUD + pipeline validation (is_won/list_deals/
# find_deal/_validate_deal/add_deal/update_deal/delete_deal/link_asset/
# unlink_asset) now live in contacts_deals_service.py (imported above).
