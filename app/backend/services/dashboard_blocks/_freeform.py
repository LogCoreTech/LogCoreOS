...
def resolve_heading_divider(ctx: BlockRenderCtx) -> BlockRenderResult:
    ...
    style = (
        ctx.config.get("style") if ctx.config.get("style") in ("heading", "divider") else "heading"
    )
    return BlockRenderResult(ok=True, data={"text": text, "style": style})


register(
    BlockSpec(type="text_block", label="Text", category="freeform", resolver=resolve_text_block)
)
register(
    BlockSpec(
        type="link_button",
...
register(
    BlockSpec(
        ...
        category="freeform",
        resolver=resolve_heading_divider,
    )
)
