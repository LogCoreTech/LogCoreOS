...
def _skip_dirs(disabled_modules: set[str]) -> set[str]:
    ...
    Tasks/Dashboards/Assets/Contacts/Business are the pre-existing
    exceptions, skipped regardless of any toggle because they're
    structurally different (Tasks/Dashboards/Assets/Contacts are JSON
    (+ binary files for Assets/Contacts), not markdown; Business is a
    nested workspace root), not module-disabled semantics.
    """
