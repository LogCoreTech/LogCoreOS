"""Tests for home's AI agent tools specifically — the real module, not a
fake one, since the interesting behavior here is Home's own double gate
(module installed AND Home Assistant actually configured) plus a real
asymmetry in agent_service.py's hardcoded tool-mode sets that this
conversion had to preserve exactly (see docs/MEMORY.md 2026-08-24)."""
...
def test_get_home_state_is_read_only_but_excluded_from_research_mode(brain):
    """A real, deliberate asymmetry predating this conversion: get_home_state
    is safe to run without approval (in _READ_TOOLS) but not offered in the
    narrower research mode (_RESEARCH_TOOLS) — preserved as a hardcoded name
    in agent_service.py rather than going through the generic
    read_only_agent_tools manifest union, which would incorrectly add it to
    both sets uniformly."""
    assert "get_home_state" in agent_service._READ_TOOLS
    assert "get_home_state" not in agent_service._RESEARCH_TOOLS


def test_control_home_device_requires_approval():
    ...
    assert "control_home_device" not in agent_service._READ_TOOLS
    assert "activate_scene" not in agent_service._READ_TOOLS
    assert "trigger_home_automation" not in agent_service._READ_TOOLS
...
def test_execute_tool_dispatches_get_home_state_through_to_ha_service(brain):
    ...
    with patch.object(ha_service, "get_state", return_value={"entity_id": "light.x", "state": "on"}) as mock_get:
        result = agent_service._execute_tool("get_home_state", {"entity_ids": ["light.x"]}, _user())
